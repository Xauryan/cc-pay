...
A Rust Playwright adapter for Alipay password payments through `cc-pay`.

The host application supplies a running `playwright_rs::Browser`. Each payment
uses a fresh, non-persistent browser context with its own request gate. The
adapter implements `cc_pay::PasswordPayer` and returns a `PasswordPayment`.
...
Before opening a context, the adapter checks the Alipay gateway URL, signature
field, API method, order fields, amount, account and payment-password format.
...
Run from the repository root. For compilation and tests with driver download
skipped:
